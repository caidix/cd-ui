<template>
  <ul v-if="totalPage >= 7" @click="handleClick($event)">
    <li :class="[{forbid: current === 1}, 'prev']">&laquo;</li>
    <li :class="{active:current === 1}">1</li>
    <li v-if="1 !== pages[0] - 1" class="default">...</li>
    <li v-for="val in pages" :key="val" :class="{active:current === val}">{{val}}</li>
    <li v-if="pages[pages.length - 1] !== totalPage - 1" class="default">...</li>
    <li :class="{active:current === totalPage}">{{totalPage}}</li>
    <li :class="[{forbid: current === totalPage}, 'next']">&raquo;</li>
  </ul>
  <ul v-else @click="handleClick($event)">
    <li :class="[{forbid: current === 1}, 'prev']">&laquo;</li>
    <li v-for="val in pages" :key="val" :class="{active: current === val}">{{val}}</li>
    <li :class="[{forbid: current === totalPage}, 'next']">&raquo;</li>
  </ul>
